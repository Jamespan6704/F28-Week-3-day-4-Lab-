...
const repeatMyParam = () => {
  axios.get("http://localhost:3000/repeat/coding-is-great").then((res) => {
...
const newQuery = () => {
  axios
    .get("http://localhost:3000/query-test?phrase=Hello-World")
    .then((res) => {
      console.log(res.data);
    });
};
...
// Added onto the other url that we had
...
// CODE HERE
const createFood = () => {
  let foodInput = document.getElementById('first')
  const body {
    newFood: pizza
  }
  axios.get('/food', body).then(() => {
    console.log(res.data)
  })
};
